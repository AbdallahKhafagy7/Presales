...
export const indexRequirementAnalyses = async (req, res) => {
  ...
  const opportunityMap = new Map(
    opportunities.map((opportunity) => [String(opportunity._id), opportunity]),
  );
  ...
  for (const analysis of analyses) {
    const opportunity = opportunityMap.get(String(analysis.opportunityId));
    if (!opportunity) {
      continue;
    }
    ...
    for (const section of ANALYSIS_SECTIONS) {
      const content = normalizeSectionContent(analysis[section.field]);
    ...
    await replaceVectors("requirement_analysis", analysis._id, documents);
